...
public class eduCollisionDetection : MonoBehaviour
{
    ...
    void CircleWallCollision(eduRigidBody body, eduWallCollider wall)
    {
        ...
        switch (wall.side)
        {
            case eduWallCollider.Wallside.Left:
                if (body.transform.position.x - circle.radius < wall.transform.position.x)
                {
                    normal = Vector2.right;
                    contactPoint = new Vector2(wall.transform.position.x, body.transform.position.y);
                    isColliding = true;
                }
                break;
            ...
            case eduWallCollider.Wallside.Bottom:
                if (body.transform.position.y - circle.radius < wall.transform.position.y)
                {
                    normal = Vector2.up;
                    contactPoint = new Vector2(body.transform.position.x, wall.transform.position.y);
                    isColliding = true;
                }
                break;
    ...
    //{
    //    eduCircleCollider circleA = bodyA.GetComponent<eduCircleCollider>();
    //    eduCircleCollider circleB = bodyB.GetComponent<eduCircleCollider>();
